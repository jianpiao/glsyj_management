...
<template>
    <section class="main">
        ...
        <div class="drag-box-left">
            ...
            >
               <span class="id">{{list.shop_category_id}}  </span>   
               <span>{{list.shop_category_name}}</span>
    ...
    export default {
        ...
        methods: {
                ...
                //  获取排序后的数据内容
                    console.log(ev.target.parentNode)
                        ...
                        for (let i = 0; i < ev.target.parentNode.children.length; i++) {
                            //  获取之后添加到和数组里面
                            this.saveSort.push({
                                shop_category_id: ev.target.parentNode.children[i].getElementsByTagName('span')[0].innerText,
                                priority: i
                            })
                        }
                        //  保存排序
            ...
            setSort() {
                ...
                this.$axios({
                    method: 'post',
                    url: self.API + 'SortShopCategoryList',
                    data: {
            ...
            deleteCategory(v) {
                ...
                this.$axios({
                    method: 'post',
                    url: self.API + 'DeleteShopCategory',
                    data: {
                        shop_category_id: v
                    },
                    headers: {
            ...
            handleDelete(i,v) {
                ...
                this.$confirm('是否确认删除分类名  [ '+ v.shop_category_name +" ]", '提示', {
                        ...
                        self.deleteCategory(v.shop_category_id)
                    }).catch(() => {
                        this.$message({
                            type: 'info',
                            message: '已取消'
